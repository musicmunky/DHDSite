<?php
	...
	if(isset($_POST['comicsubmit']) && $_POST['comicsubmit'] == "Upload")
	{
		...
		{
			//print_r($info);
			//$i = mysql_real_escape_string($info);
			echo "<script type='text/javascript'>
					alert('There was an error during file upload - try again, but if it keeps happening, call Tim');
					window.top.window.finishUpload(0,0);
				</script>";
		}
	}
	...
	{
		echo "<script type='text/javascript'>
					alert('Unable to upload your file at this time - please contact your administrator');
					window.top.window.finishUpload(0,0);
				</script>";
	}
?>